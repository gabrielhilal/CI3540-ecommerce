class UsersController < ApplicationController
  ...
  #                                                          no current user current_user.admin? doesn't exist.
  def index
  ...
  def show
    ...
    #so first we check if the someone is logged
    unless current_user.nil?
      #if someone is logged we need to check if the user if admin
      if current_user.admin?
        #if the user is admin we find the user by its id
  ...
  def new
    ...
    if current_user.nil?
      ...
      #we need to check if the current user is admin, as only logged users with admin privileges can
      #                                                                             create new users.
      if current_user.admin?
      ...
      #                   asking the user to logout and redirect to the INDEX application (store_path).
      else
  ...
  def create
    ...
    if @user.save
      ...
      #        INDEX for details. Display success message and redirect to the INDEX action (users_path).
      else
