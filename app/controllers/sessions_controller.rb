...
#the sessions controller is based on the book: Ruby on Rails Tutorial-Learn Rails by Example (Michael Hartl)-http://ruby.railstutorial.org/,
# full reference details are available in the report's reference list.
  def new
    ...
    unless current_user.nil?
      #if current_user is not nil (someone is logged in) we redirect to the application INDEX (store_path)
  ...
  user = User.find_by_email(params[:email])
    ...
    if user && user.authenticate(params[:password])
      ...
      # display the confirmation message and redirect to the application INDEX (store_path). Please note the application controller for details.
